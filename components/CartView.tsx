...
import { elevationSvg, birdsEyeSvg } from '@/lib/closet-svg';
import { hardwarePills } from '@/lib/closet-geometry';
import { finishedHeightLabel, wallsForShape } from '@/lib/config';
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {items.map((item, i) => {
            ...
            const pills = hardwarePills(catalog, cfg);
            const elevation = elevationSvg(catalog, cfg);
            const birdseye = birdsEyeSvg(catalog, cfg);
            ...
            return (
              <div key={item.id} style={{ background: '#fff', borderRadius: 12, border: `0.5px solid ${CARD_BORDER}`, padding: 32 }}>
                ...
                {/* hardware pills (§5 conditional logic) */}
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: elevation }}
                />
                ...
                <div
                  className="[&_svg]:h-auto [&_svg]:max-w-full"
                  style={{ background: OFFWHITE, borderRadius: 8, padding: 14, marginBottom: 4, display: 'flex', justifyContent: 'center' }}
                  dangerouslySetInnerHTML={{ __html: birdseye }}
                />
